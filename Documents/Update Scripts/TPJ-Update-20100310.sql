alter table pys_pagibig_contri_dtl drop constraint pphd_fk;
alter table pys_pagibig_contri_hdr drop primary key;

alter table pys_pagibig_contri_hdr drop column PAYROLL_NO;
alter table pys_pagibig_contri_dtl drop column PPCH_PAYROLL_NO;

alter table pys_pagibig_contri_hdr add (
   cv_no number(12,0),
   cv_date number(12,0),
   or_amt number(12,2) default 0 not null
);

alter table pys_pagibig_contri_hdr add
   constraint pphd_pk primary key (period_fr, period_to);

alter table pys_health_contri_dtl drop constraint phcd_fk;
alter table pys_health_contri_hdr drop primary key;

alter table pys_health_contri_hdr drop column PAYROLL_NO;
alter table pys_health_contri_dtl drop column PPCH_PAYROLL_NO;

alter table pys_health_contri_hdr add (
   cv_no number(12,0),
   cv_date number(12,0),
   or_amt number(12,2) default 0 not null
);
...
alter table pys_health_contri_hdr add
   constraint phcd_pk primary key (period_fr, period_to);


alter table pys_sss_contri_hdr modify cv_date date;
alter table pys_pagibig_contri_hdr modify cv_date date;
alter table pys_health_contri_hdr modify cv_date date;

delete from pys_sss_contri_hdr;

INSERT INTO pys_sss_contri_hdr (PERIOD_FR, PERIOD_TO, CREATED_BY, DT_CREATED )
SELECT PERIOD_FR, PERIOD_TO, USER, SYSDATE
FROM   PYS_SSS_CONTRIBUTION
GROUP  BY PERIOD_FR, PERIOD_TO, USER, SYSDATE;

INSERT INTO pys_pagibig_contri_hdr (PERIOD_FR, PERIOD_TO, CREATED_BY, DT_CREATED )
SELECT PERIOD_FR, PERIOD_TO, USER, SYSDATE
FROM   PYS_PAGIBIG_CONTRIBUTION
GROUP  BY PERIOD_FR, PERIOD_TO, USER, SYSDATE;


INSERT INTO pys_health_contri_hdr (PERIOD_FR, PERIOD_TO, CREATED_BY, DT_CREATED )
SELECT PERIOD_FR, PERIOD_TO, USER, SYSDATE
FROM   PYS_PHILHEALTH_CONTRIBUTION
GROUP  BY PERIOD_FR, PERIOD_TO, USER, SYSDATE;
