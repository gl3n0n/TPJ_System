         ...
         if i.adj_flag ='Y' then
            ...
            if bIsEndOfMonth then
               ...
               if i.no_days > 0 then
                  ...
                  dAEnd   := p_date_fr;
                  nANumDay := i.no_days;
                  if dAStart between (p_date_fr-5) and p_date_fr then
                     dPrevRateG := 0;
                     dPrevRateR := 0;
                  end if;
               end if;
            ...
            set    period_fr = dAStart,
                   ...
                   amt_g   = nvl(abs(nvl(dPrevRateG,0) - decode(sal_freq,'MONTHLY',i.basic_rate_g/30,i.basic_rate_g)) * nADays,0),
                   amt     = nvl(nAdjPay,0),
                   no_days = nADays
            ...
            if p_empl_id in (dEmplID) then
               dbms_output.put_line ('check 5: ' || p_empl_id || ': dAStart=' || to_char(dAStart) || ':dAEnd-1=' || to_char(dAEnd-1) || 
