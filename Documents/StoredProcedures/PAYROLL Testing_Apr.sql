begin
   delete from pys_sss_contri_dtl where psch_payroll_no = 20080415;
   delete from pys_pagibig_contri_dtl where ppch_payroll_no = 20080415;
   delete from pys_health_contri_dtl where phch_payroll_no = 20080415;                
   delete from PYS_PAYROLL_DTL where pahd_payroll_no = 20080415;       
   delete from PYS_PAYROLL_DTL_LOG where payroll_no = 20080415;       
   commit;
   sp_payroll_computation_a (20080415, '2008','04',to_date('20080401','YYYYMMDD'),to_date('20080415','YYYYMMDD')); 
   commit;
end;

select * from PYS_PAYROLL_DTL_LOG where empl_empl_id = 'M00002';

begin
   delete from pys_sss_contribution where period_to = to_date('20080430','YYYYMMDD');
   delete from pys_pagibig_contribution where period_to = to_date('20080430','YYYYMMDD');
   delete from pys_philhealth_contribution where period_to = to_date('20080430','YYYYMMDD');

   delete from pys_sss_contri_dtl where psch_payroll_no = 20080430;
   delete from pys_pagibig_contri_dtl where ppch_payroll_no = 20080430;
   delete from pys_health_contri_dtl where phch_payroll_no = 20080430;                
   delete from PYS_PAYROLL_DTL where pahd_payroll_no = 20080430;       
   delete from PYS_PAYROLL_DTL_LOG where payroll_no = 20080430;
   commit;
...
select * from pys_sss_contribution where period_to = to_date('20080430','YYYYMMDD')
   ...
   where  vocr.voya_voyage_date <= to_date('20080426','YYYYMMDD')
   and    vocr.dt_embarked <= to_date('20080426','YYYYMMDD')
   and    vocr.dt_disembarked is null 
   and    vocr.voya_vess_code = vess.code
   and    vocr.empl_empl_id = empl.empl_id
   and    vocr.empl_empl_id = 'A00002'
   ...
   and    vocr.dt_disembarked <> (to_date('20080426','YYYYMMDD')-1) -- end of month
   and    vocr.voya_vess_code = vess.code
   and    vocr.empl_empl_id = empl.empl_id
   and    vocr.empl_empl_id = 'A00002'
   and    vocr.passenger = 'N'
   ...
   select vocr.empl_empl_id, vocr.rowid row_id, vocr.dt_embarked, vocr.dt_disembarked,
          ...
          vocr.voya_voyage_date, vocr.basic_rate, vocr.basic_rate_g
   from   cms_voyage_crew vocr, cms_vessels vess, pms_employees empl
   where  vocr.voya_vess_code = vess.code
   and    vocr.empl_empl_id = empl.empl_id
   and    vocr.empl_empl_id = 'A00002'
   and    vocr.passenger = 'N'
   order  by dt_embarked desc
...
select * from pys_payroll_dtl where empl_empl_id = 'H00004'
...
select BASIC_RATE_G, basic_rate, dept_code, SAL_FREQ, no_days, period_to
                      from   pys_payroll_dtl
                      where  empl_empl_id = 'A00003'
                      and    pahd_payroll_no = 20080430
                      and    paty_code like 'REG%'
                      union
                      select BASIC_RATE_G, basic_rate, dept_code, SAL_FREQ, no_days, period_to
                      from   pys_payroll_dtl
                      where  empl_empl_id = 'A00003'
                      and    pahd_payroll_no = 20080415
                      and    paty_code like 'REG%'
