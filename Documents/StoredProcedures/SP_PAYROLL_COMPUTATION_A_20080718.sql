      ...
      if i.taty_code is not null then         -- START: Tax Type checking (no Deduction loop)
         ...
         if i.empl_type = 'OFC' then
            ...
            sp_count_flt_attendance ( i.empl_empl_id, p_payno, p_year, p_mon, p_date_fr, p_date_to,
                                      ...
                                      nSeqNo, dEmplID, vSalFreq, nNumHrs, nSalaryG, nSuPay, nHoPay, nHSPay, nAllowances, vLatestVess, vLatestTitle, nSeqNo);
         end if;
            ...
            loop
               ...
               if k.sal_freq = 'MONTHLY' then
                  if k.dept_code='FL' then
                     ...
                     --nSalaryG_B := k.BASIC_RATE_G;  -- check crew
                  else
               ...
               if i.empl_type = 'OFC' then
                  ...
                  if nVale < 0 then
                     begin
                        ...
                        values (DEDU_SEQ.NEXTVAL, i.empl_empl_id, 'VALE', p_date_to, p_date_to+1, 1, nVale, 'MONTHLY', abs(nVale), sysdate, user);
                     end;
                  end if;
               end if;
