   ...
   for j in att_flt(dFirstDay, dEnd) loop
      ...
      then
         ...
         if j.dt_disembarked < p_date_fr then
            ...
            if j.dt_disembarked < dFirstday then         -- end of feb... feb 13 disbarked scenario
               dEnd := p_date_fr;
            else
               dEnd := j.dt_disembarked;
            end if;                                      -- end of feb... feb 13 disbarked scenario
            if j.dt_embarked > dFirstDay then
               dStart := j.dt_embarked;
            else
               if j.dt_disembarked < p_date_fr then     -- 15 of mar... feb 26 disbarked scenario
                  dStart := dFirstDay;
               else
                  dStart := dFirstDay-1;
               end if;
            end if;                                     -- 15 of mar... feb 26 disbarked scenario
            -- for feb changes
            vAdjFlag := 'Y';
            bAdjustment := TRUE;
         end if;
