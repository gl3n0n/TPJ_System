...
begin
   ...
   for j in att_flt(dFirstDay, dEnd) loop
      ...
      if (nLastNumDay+nLastRegSun+nLastRegHol+nLastHolSun) <> 0 then
         ...
         values ( p_payno, p_year, p_mon, dStart, last_day(dStart), nSeqNo, j.empl_empl_id, vLastPosi, vLastTitl, 'REG-ADJ', nLastPay*-1, nLastNumDay*-1, nLastRate, vLastVess, 'FL', user, sysdate, 'ADD', p_sal_freq, vLatestVess );
      end if;
