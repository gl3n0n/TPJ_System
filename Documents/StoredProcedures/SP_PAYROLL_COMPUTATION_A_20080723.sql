...
begin
   ...
   for j in atre ( p_empl_id, dStart, dEnd ) loop
      ...
      if p_empl_id = dEmplID then
         if bNoOuterAdj then
            dbms_output.put_line ('check 2outeradj: j.tx_date: ' || to_char(j.tx_date) || ' j.num_hours: ' || to_char(j.num_hours) || ' j.outer_port:' || j.outer_port || ' j.ot_hours:' || to_char(j.ot_hours) || ',nNumHrs:' || to_char(nNumHrs));
         else
   ...
   if nOuterAdD > 0 then
      ...
      insert into pys_payroll_dtl 
             ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, paty_code, amt, no_days, basic_rate, amt_g, basic_rate_g, vess_code, dept_code, created_by, dt_created, pay_flag, sal_freq )
      values ( p_payno, p_year, p_mon, dOuterFrD, dOuterToD, nSeqNo, p_empl_id, p_posi_code, 'REG-OP-ADJ', p_basic_r*nOuterAdD, p_basic_r*nOuterAdD, p_basic_r, nOPAdjD, p_basic_g, null, p_dept_code, user, sysdate, 'ADD', vSalFreq );
   end if;
