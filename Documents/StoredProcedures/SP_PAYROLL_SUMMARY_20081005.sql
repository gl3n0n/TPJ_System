   ...
   if to_char(dStart, 'DD') = '16' then 
      ...
      loop 
         ...
         loop 
            ...
            -- compute for 13th MONTH and SILP
            n13th_n_mon := 0;
            n13th_a := 0;  nSILP_a := 0;  n13th_amt_a := 0;  
            n13th_b := 0;  nSILP_b := 0;  n13th_amt_b := 0;
            for m in (select max(pay.cola_rate) cola_rate, 
                             sum(pay.no_days) no_days, 
                             greatest(sum(pay.cola_amt),0) cola_amt, 
                             sum(pay.amount) amount, 
                             sum(pay.amount_g) amount_g, 
                             max(pay.sal_freq) sal_freq
                      from   pys_payroll_summary pay
                      where  pay.empl_id like i.empl_id
                      and    pay.payroll_no in (nPrevPayrollNo, p_payroll_no ) )
            loop
               if i.dept_code in ('FL', 'MA-CREW') then
                  nAmt_a := m.amount;
                  insert into pys_13th_month 
                         (empl_id, payroll_no, period_to, basic_rate, basic_rate_a, amt, amt_a, cola_rate, cola_amt, no_days, dept_code, vess_code, title, oport ) 
                  values 
                         (i.empl_id, p_payroll_no, dEnd, k.basic_rate, k.basic_rate_g, m.amount,  nAMt_a, m.cola_rate, m.cola_amt, m.no_days, i.dept_code, k.l_vess_code, nvl(k.l_title, k.title), 'N' );                
               else
                  select nvl(sum(no_days),0) no_days
                  into   nOP_Days
                  from   pys_payroll_dtl
                  where  empl_empl_id = i.empl_id
                  and    paty_code like 'REG-OP'
                  and    pahd_payroll_no in (nPrevPayrollNo, p_payroll_no);

                  nAmt_a := k.basic_rate_g;
                  nBasic := 0;
                  if nOP_Days > 0 then
                     for o in (select basic_rate, sal_freq
                               from   pys_payroll_dtl
                               where  empl_empl_id = i.empl_id
                               and    paty_code like 'REG-OP'
                               and    pahd_payroll_no in (nPrevPayrollNo, p_payroll_no)
                               order  by period_to desc )
                     loop
                        if o.sal_freq = 'MONTHLY' then
                           nOP_Basic := o.basic_rate/30;
                        else 
                           nOP_Basic := o.basic_rate;
                        end if;
                        exit; 
                     end loop;
                     ...
                     insert into pys_13th_month 
                            (empl_id, payroll_no, period_to, basic_rate, basic_rate_a, amt, amt_a, cola_rate, cola_amt, no_days, dept_code, vess_code, title, oport ) 
                     values 
                            (i.empl_id, p_payroll_no, dEnd, nOP_Basic/nOuterR, k.basic_rate_g, (nOP_Basic/nOuterR)*nOP_Days, nAMt_a, 0, 0, nOP_Days, i.dept_code, k.l_vess_code, nvl(k.l_title, k.title), 'Y' );

                     for o in (select basic_rate, sal_freq
                               from   pys_payroll_dtl
                               where  empl_empl_id = i.empl_id
                               and    paty_code like 'REG%'
                               and    paty_code not like 'REG-OP'
                               and    pahd_payroll_no in (nPrevPayrollNo, p_payroll_no)
                               order  by period_to desc )
                     loop
                        if o.sal_freq = 'MONTHLY' then
                           nBasic := k.basic_rate/30;
                        else 
                           nBasic := k.basic_rate;
                        end if;
                        exit; 
                     ...
                     insert into pys_13th_month 
                            (empl_id, payroll_no, period_to, basic_rate, basic_rate_a, amt, amt_a, cola_rate, cola_amt, no_days, dept_code, vess_code, title, oport )
                     values 
                             (i.empl_id, p_payroll_no, dEnd, nBasic, k.basic_rate_g, nBasic*(m.no_days-nOP_Days), nAMt_a, m.cola_rate, m.cola_amt, m.no_days-nOP_Days, i.dept_code, k.l_vess_code, nvl(k.l_title, k.title), 'N' );
                  else
                     if m.sal_freq = 'MONTHLY' then
                        if k.basic_rate > 10000 then
                           nBasic := k.basic_rate/30;
                        else
                           nBasic := k.basic_rate;
                        end if;
                     ...
                     insert into pys_13th_month 
                            (empl_id, payroll_no, period_to, basic_rate, basic_rate_a, amt, amt_a, cola_rate, cola_amt, no_days, dept_code, vess_code, title, oport )
                     values 
                             (i.empl_id, p_payroll_no, dEnd, nBasic, k.basic_rate_g, m.amount, nAMt_a, m.cola_rate, m.cola_amt, m.no_days, i.dept_code, k.l_vess_code, nvl(k.l_title, k.title), 'N' );
                  end if;
               end if;
            end loop;
            ...
            for n in (select pay.basic_rate basic_rate_b, 
                             pay.basic_rate_a basic_rate_a, 
                             pay.amt amt_b, 
                             pay.amt_a amt_a, 
                             pay.no_days no_days, 
                             pay.cola_amt cola_amt,
                             decode(greatest(pay.no_days,15),15,.5,1) nMon, oport
                      from   pys_13th_month pay
                      where  pay.empl_id = i.empl_id
                      and    pay.period_to between dPERIODFR and dPERIODTO )
            loop
               n13th_n_mon := n13th_n_mon + n.nMon;
               n13th_amt_b := n13th_amt_b + (n.amt_b + n.cola_amt);
               if n.oport = 'N' then 
                  n13th_amt_a := n13th_amt_a + (n.amt_a + n.cola_amt);
               end if;
            end loop;
            n13th_a := (n13th_amt_a/n13th_n_mon) * (n13th_n_mon/12);
            n13th_b := (n13th_amt_b/n13th_n_mon) * (n13th_n_mon/12);
            
            for n in (select pay.basic_rate,
                             sum(decode(greatest(pay.no_days,15),15,.5,1)) nMon
                      from   pys_13th_month pay
                      where  pay.empl_id = i.empl_id
                      and    pay.period_to between dPERIODFR and dPERIODTO
                      group  by  pay.basic_rate)
            loop
               nSILP_b := nSILP_b + n.basic_rate * 5 * (n.nMon/12);
            end loop;
            
            for n in (select decode(pay.dept_code,'FL',pay.basic_rate_a,'MA-CREW',pay.basic_rate_a,pay.basic_rate_a/30) basic_rate,
                             sum(decode(greatest(pay.no_days,15),15,.5,1)) nMon
                      from   pys_13th_month pay
                      where  pay.empl_id = i.empl_id
                      and    pay.period_to between dPERIODFR and dPERIODTO
                      group  by  decode(pay.dept_code,'FL',pay.basic_rate_a,'MA-CREW',pay.basic_rate_a,pay.basic_rate_a/30))
            loop
