...
begin
   ...
   loop
      ...
      if j.oport = 'Y' then
         ...
         then
            if j.nActDay < 15 then
               nDays := 15 + (j.nNumday-j.nActDay);
            else
               nDays := j.nActDay + (j.nNumday-j.nActDay);
            end if;
      ...
      if j.nCola > 0 then
         ...
         values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.posi_code, j.title, 'COLA', j.nCola, j.nNumDay, j.nCola/j.nNumDay, j.dept_code, user, sysdate, 'ADD', vSalFreq );
      end if;
   ...
   loop
      ...
      if j.basic_rate = 0 and j.a_basic_rate > 0 then
         ...
         if (j.nACola-j.nCola) > 0 then
            ...
            insert into pys_payroll_dtl
                   ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, dept_code, created_by, dt_created, pay_flag, sal_freq )
            values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.a_posi_code, j.a_title, 'COLA', j.nACola, j.nANumDay, j.nACola/j.nANumDay, j.a_dept_code, user, sysdate, 'ADD', vSalFreq );
         end if;
      else
            ...
            if j.a_oport = 'Y' then
               if j.a_oport <> j.oport then
                  ...
                  if j.basic_rate > 0 and j.a_basic_rate = 0 then
                     ...
                     insert into pys_payroll_dtl
                            ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, amt_g, basic_rate_g, dept_code, created_by, dt_created, pay_flag, adj_flag, sal_freq )
                     values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.a_posi_code, j.a_title, 'REG-OP-ADJ', (j.nASalaryR-j.nSalaryR)+(j.nASuPay-j.nSuPay)+(j.nAHoPay-j.nHoPay)+(j.nAHSPay-j.nHSPay), j.nANumDay+j.nASuDays+j.nAHoDays+j.nAHSDays, (j.a_basic_rate-j.basic_rate), p_basic_g/2, p_basic_g, j.a_dept_code, user, sysdate, 'ADD', 'N', vSalFreq );
                  end if;
               end if;
            else
               if j.a_oport <> j.oport then
                  ...
                  if j.basic_rate > 0 and j.a_basic_rate = 0 then
                     ...
                     if (j.nNumday - j.nANumday) <> 0 then
                        nSeqNo  := nSeqNo + 1;
                        insert into pys_payroll_dtl
                               ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, amt_g, basic_rate_g, dept_code, created_by, dt_created, pay_flag, adj_flag, sal_freq )
                        values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.a_posi_code, j.a_title, 'REG-ADJ', (j.nASalaryR)+(j.nASuPay)+(j.nAHoPay)+(j.nAHSPay), j.nANumDay+j.nASuDays+j.nAHoDays+j.nAHSDays, j.a_basic_rate, p_basic_g/2, p_basic_g, j.a_dept_code, user, sysdate, 'ADD', 'N', vSalFreq );
                        ...
                        insert into pys_payroll_dtl
                               ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, amt_g, basic_rate_g, dept_code, created_by, dt_created, pay_flag, adj_flag, sal_freq )
                        values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.posi_code, j.title, 'REG-ADJ', (j.nSalaryR+j.nSuPay+j.nHoPay+j.nHSPay)*-1, (j.nNumDay+j.nSuDays+j.nHoDays+j.nHSDays)*-1, j.basic_rate, p_basic_g/2, p_basic_g, j.dept_code, user, sysdate, 'ADD', 'N', vSalFreq );
                     else
                        nSeqNo  := nSeqNo + 1;
                        insert into pys_payroll_dtl
                               ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, amt_g, basic_rate_g, dept_code, created_by, dt_created, pay_flag, adj_flag, sal_freq )
                        values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.a_posi_code, j.a_title, 'REG-ADJ', (j.nASalaryR-j.nSalaryR)+(j.nASuPay-j.nSuPay)+(j.nAHoPay-j.nHoPay)+(j.nAHSPay-j.nHSPay), j.nANumDay+j.nASuDays+j.nAHoDays+j.nAHSDays, j.a_basic_rate, p_basic_g/2, p_basic_g, j.a_dept_code, user, sysdate, 'ADD', 'N', vSalFreq );
                     end if;
                  end if;
               end if;
            end if;
