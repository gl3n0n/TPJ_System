   ...
   for j in att_flt(dFirstDay, dEnd) loop
      ...
      begin
         ...
         if p_sal_freq = 'MONTHLY' then 
            ...
            nSunday_R  := ( p_Sunday_RF * (j.basic_rate_g/30) );
            nHoliday_R := ( p_Holiday_RF * (j.basic_rate_g/30) );
            nHolSun_R  := ( p_HolSun_RF * (j.basic_rate_g/30) );
         else
         ...
         values ( p_payno, p_year, p_mon, p_date_fr, dEnd, nSeqNo, j.empl_empl_id, j.posi_code, j.title, vPaty, nSalaryR, nNumDay, j.basic_rate, nSalaryG, decode(j.basic_rate_g,0,j.basic_rate,j.basic_rate_g), j.voya_vess_code, 'FL', user, sysdate, 'ADD', vAdjFlag, p_sal_freq );
      ...
      if bIsEndOfMonth then
         if nRegSun > 0 then
            ...
            insert into pys_payroll_dtl
                   ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, vess_code, dept_code, created_by, dt_created, pay_flag, sal_freq )
            values ( p_payno, p_year, p_mon, dStart, dEnd, nSeqNo, j.empl_empl_id, j.posi_code, j.title, 'OT-SUN-FLT', nSuPay, nRegSun, nSunday_R, j.voya_vess_code, 'FL', user, sysdate, 'ADD', p_sal_freq );
         end if;
         ...
         if nRegHol > 0 then
            ...
            insert into pys_payroll_dtl
                   ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, vess_code, dept_code, created_by, dt_created, pay_flag, sal_freq )
            values ( p_payno, p_year, p_mon, dStart, dEnd, nSeqNo, j.empl_empl_id, j.posi_code, j.title, 'OT-HOL-FLT', nHoPay, nRegHol, nHoliday_R, j.voya_vess_code, 'FL', user, sysdate, 'ADD', p_sal_freq );
         end if;
         ...
         if nHolSun > 0 then
            ...
            insert into pys_payroll_dtl
                   ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, vess_code, dept_code, created_by, dt_created, pay_flag, sal_freq )
            values ( p_payno, p_year, p_mon, dStart, dEnd, nSeqNo, j.empl_empl_id, j.posi_code, j.title, 'OT-HS-FLT', nHSPay, nHolSun, nHolSun_R, j.voya_vess_code, 'FL', user, sysdate, 'ADD', p_sal_freq );
         end if;
      end if;
      ...
      if nRecCtr = 0 then
         ...
         for x in allo_flt ( j.empl_empl_id, j.voya_vess_code,  j.voya_voyage_date  ) loop
            ...
            insert into pys_payroll_dtl
                   ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, vess_code, dept_code, created_by, dt_created, pay_flag, sal_freq )
            values ( p_payno, p_year, p_mon, p_date_fr, p_date_to, nSeqNo, j.empl_empl_id, j.posi_code, j.title, x.allo_code, (x.amt*nNumDay), nNumDay, x.amt, j.voya_vess_code, 'FL', user, sysdate, 'ADD', p_sal_freq );
         end loop;
      end if;
      ...
      for i in adj_flt loop
         ...
         if i.adj_flag ='Y' then
            ...
            set    period_fr = dAStart,
                   ...
                   amt_g   = nvl((dPrevRateG - decode(sal_freq,'MONTHLY',i.basic_rate_g/30,i.basic_rate_g)) * i.no_days,0),
                   amt     = nvl(nAdjPay,0),
                   no_days = nADays
