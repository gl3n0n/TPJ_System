...
begin
   ...
   loop
      ...
      if j.nCola > 0 then
         ...
         values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.posi_code, j.title, 'COLA', j.nCola, j.nNumday, j.nCola/j.nNumday, j.vess_code, 'FL', user, sysdate, 'ADD', p_sal_freq, vLatestVess );
      end if;
   ...
   if bIsEndOfMonth then
      ...
      loop
         ...
         if j.basic_rate = 0 and j.a_basic_rate > 0 then
            ...
            if (j.nACola-j.nCola) > 0 then
               ...
               insert into pys_payroll_dtl
                      ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, vess_code, dept_code, created_by, dt_created, pay_flag, sal_freq, Latest_Vess )
               values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.a_posi_code, j.a_title, 'OT-HS-FLT', j.nACola, j.nANumDay, j.nACola/j.nANumDay, j.a_vess_code, 'FL', user, sysdate, 'ADD', p_sal_freq, vLatestVess );
            end if;
         else
            if (j.basic_rate-j.a_basic_rate) <> 0 then
               if j.basic_rate > 0 and j.a_basic_rate = 0 then
                  ...
                  insert into pys_payroll_dtl
                         ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, amt_g, basic_rate_g, vess_code, dept_code, created_by, dt_created, pay_flag, adj_flag, sal_freq, latest_vess )
                  values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.a_posi_code, j.a_title, 'REG-ADJ', (j.nASalaryR-j.nSalaryR), j.nNumDay, (j.a_basic_rate-j.basic_rate), (j.nASalaryR-j.nSalaryR), (j.a_basic_rate-j.basic_rate), j.a_vess_code, 'FL', user, sysdate, 'ADD', 'N', p_sal_freq, vLatestVess );
               end if;
            end if;
      ...
      loop
         ...
         if j.basic_rate = 0 and j.a_basic_rate > 0 then
            ...
            insert into pys_payroll_dtl
                   ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, amt_g, basic_rate_g, vess_code, dept_code, created_by, dt_created, pay_flag, adj_flag, sal_freq, latest_vess )
            values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.a_posi_code, j.a_title, 'REG-ADJ', j.nASalaryR+j.nASuPay+j.nAHoPay+j.nAHSPay, j.nANumDay+j.nASuDays+j.nAHoDays+j.nAHSDays, j.a_basic_rate, j.nASalaryR+j.nASuPay+j.nAHoPay+j.nAHSPay, j.a_basic_rate, j.a_vess_code, 'FL', user, sysdate, 'ADD', 'N', p_sal_freq, vLatestVess );
      
            if (j.nACola-j.nCola) > 0 then
               nSeqNo := nSeqNo + 1;
               insert into pys_payroll_dtl
                      ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, vess_code, dept_code, created_by, dt_created, pay_flag, sal_freq, Latest_Vess )
               values ( p_payno, p_year, p_mon, j.dStart, j.dEnd, nSeqNo, p_empl_id, j.a_posi_code, j.a_title, 'COLA', j.nACola, j.nANumDay, j.nACola/j.nANumDay, j.a_vess_code, 'FL', user, sysdate, 'ADD', p_sal_freq, vLatestVess );
            end if;
         else
            if (j.basic_rate-j.a_basic_rate) <> 0 then
               if j.basic_rate > 0 and j.a_basic_rate = 0 then
