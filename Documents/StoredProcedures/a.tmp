...
begin
   ...
   if p_date_to <= to_date(to_char(p_date_to, 'YYYYMM') || '15', 'YYYYMMDD') then
      ...
      select max(pahd_payroll_no)
      into   nPayNo
      from   pys_payroll_dtl;

      if nPayNo is null then
            raise_application_error (-20001, 'ERROR - no data on payroll dtl - payNo:' || to_char(p_payno)  || ' - dateFr:' || to_char(p_date_fr) || ' - dateTo:' || to_char(p_date_to)); 
      end if;
      ...
      begin
         ...
         where  payroll_no = nPayNo;
      exception
         when NO_DATA_FOUND then
            raise_application_error (-20001, 'ERROR - retrieval of latest payroll no ' || to_char(nPayNo) || ' dated ' || to_char(p_date_fr)); 
      end;
   ...
   for i in attr ( dPrevStart, p_date_to ) loop
      ...
      if i.taty_code is not null then         -- START: Tax Type checking (no Deduction loop)
         ...
         begin
            nSeqNo  := nSeqNo + 1;
            if vSalFreq = 'MONTHLY' and i.empl_type = 'FLT' then
               nSalaryR := nBasicR/2;
               nSalaryG := nBasicG/2;
            else
         ...
         if (bWithDeduction) and (nvl(nBasicG,0) > 0) then  
            ...
            begin
               nSalaryG   := nSalaryG + nvl(nOPPay,0)  + nvl(nOPAdj,0) + nvl(nOtPay,0) + nvl(nSuPay,0) + nvl(nHoPay,0) + nvl(nHsPay,0) + nvl(nAllowances,0) + nvl(nPrevSal,0) + nvl(nPrevAllo,0);
            end;
            ...
            exception
               when OTHERS then
                  raise_application_error (-20001, SQLERRM || ' ERROR - sss contribution for ' || i.empl_empl_id || ' period: ' || to_char(dPrevStart) || '-' || to_char(p_date_to)); 
