...
begin
  ...
  for a in
    (
    select d.acco_code, d.debit debit_php, d.credit credit_php, --h.particular,
           h.cv_no, h.cv_date
  ...
  loop
    ...
    if a.acco_code = '10002' then
       ...
       insert into acc_cv_summary_sundry(cv_no, sundry, debit, credit, userid)
       values (a.cv_no, sf_acc_get_account_name(a.acco_code), v_sun_deb_amt, v_sun_cre_amt, user);
    end if;
