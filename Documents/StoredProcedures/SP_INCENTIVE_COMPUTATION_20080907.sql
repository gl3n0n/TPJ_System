      ...
      if i.passenger <> 'Y' then
            ...
            for j in (select code from cms_fishing_sources where status = 'ACTIVE') loop
               ...
               exception
                  when no_data_found then
                     sp_insert_employee_incentive ( i.empl_empl_id, p_year, p_mon, dStart, dEnd, 'CATCHER', i.vessel, i.rank_code, j.code, 0, 0, 0, p_tranno );
               end;
            end loop;
         end if;
         ...
         for j in dcsu_l ( i.vessel, dStart, dEnd ) loop
            if j.vess_lighted = j.vess_surveyed then
               ...
               if j.total_catch > 0 and nRate > 0 then
                  sp_insert_employee_incentive ( i.empl_empl_id, p_year, p_mon, dStart, dEnd, 'LIGHTBOAT', i.vessel, i.rank_code, j.fiso_code, j.total_catch, nRate, j.total_catch*nRate, p_tranno );
               end if;
               nLIGHTBOAT := nLIGHTBOAT + (j.total_catch*nRate);
            else
               nRate := sf_get_lighted_rate ( j.fiso_code, i.rank_code, nTotalCatch/2);
               if j.total_catch > 0 and nRate > 0 then
                  sp_insert_employee_incentive ( i.empl_empl_id, p_year, p_mon, dStart, dEnd, 'LIGHTBOAT', i.vessel, i.rank_code, j.fiso_code, j.total_catch/2, nRate, (j.total_catch/2)*nRate, p_tranno );
               end if;
         ...
         if nLIGHTBOAT > 0 then
            for j in (select code from cms_fishing_sources where status = 'ACTIVE') loop
               ...
               exception
                  when no_data_found then
                     sp_insert_employee_incentive ( i.empl_empl_id, p_year, p_mon, dStart, dEnd, 'LIGHTBOAT', i.vessel, i.rank_code, j.code, 0, 0, 0, p_tranno );
               end;
            end loop;
         end if;
