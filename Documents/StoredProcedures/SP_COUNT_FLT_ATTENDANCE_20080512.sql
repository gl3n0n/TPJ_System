...
begin  
   ...
   for j in att_flt(dFirstDay, dEnd) loop
      ...
      if j.dt_disembarked is null then
         dEnd := p_date_to; --dEnd;
      elsif j.dt_disembarked > p_date_to then --dEnd then
         dEnd := p_date_to;
      elsif j.dt_disembarked < p_date_to then --dEnd then
      ...
      --dbms_output.put_line ('check 2: start' || to_char(dStart) || ': End=' || to_char(dEnd) || ': Day=' || to_char((dEnd-dStart)) || ': RecCtr=' || to_char(nRecCtr));
      -- check total days
      ...
      begin
         ...
         values ( p_payno, p_year, p_mon, dStart, dEnd, nSeqNo, j.empl_empl_id, j.posi_code, j.title, vPaty, nSalaryR, nNumDay, j.basic_rate, nSalaryG, j.basic_rate_g, j.voya_vess_code, null, user, sysdate, 'ADD', vAdjFlag );
