...
begin
   ...
   loop
      ...
      if p_dEmplID = p_empl_id then
         dbms_output.put_line('Dito ako... dPeriodFr=' || to_char(dPeriodFr) || ',j.nColaPay=' || to_char(j.nColaPay) ||
                              ',j.nNumDays=' || to_char(j.nNumDays) || ',j.nColaDay=' || to_char(j.nColaDay));
      end if;
      if j.nColaPay > 0 and j.nColaDay > 0 then
         nSeqNo := nSeqNo + 1;
         if p_isMonthly = 'Y' and j.nNumDays >= 15 then
            insert into pys_payroll_dtl
            ...
            nTotalCola := nTotalCola + j.nColaDay;
         end if;
      end if;
