...
(
   ...
   cursor last_flt (p_end in date) is
   select vocr.empl_empl_id, vocr.rowid row_id, vocr.dt_embarked, vocr.dt_disembarked,
          vocr.passenger, vocr.voya_vess_code, empl.posi_code posi_code, vocr.title,
          vocr.voya_voyage_date, vocr.basic_rate, vocr.basic_rate_g
   from   cms_voyage_crew vocr, cms_vessels vess, pms_employees empl
   where  vocr.voya_voyage_date <= p_end
   and    vocr.dt_disembarked <= p_end
   and    vocr.voya_vess_code = vess.code
   and    vocr.empl_empl_id = empl.empl_id
   and    vocr.empl_empl_id = p_empl_id
   and    vocr.passenger = 'N';
   ...
   --get allowances
   cursor allo_flt (p_empl_id in varchar2, p_effectivity in date) is
   ...
   nLastRate   Number(8,2) := 0;
   vLastPosi   Varchar2(32);
   vLastTitl   Varchar2(32);
   vLastVess   Varchar2(32);
   nLastNumDay Number := 0;
   nLastHolSun Number := 0;
   nLastRegHol Number := 0;
   nLastRegSun Number := 0;
   nLastPay    Number(8,2) := 0;
   nLastSuPay  Number(8,2) := 0;
   nLastHoPay  Number(8,2) := 0;
   nLastHSPay  Number(8,2) := 0;

   nChkReg     Number;
   dLastDisembarked Date;
...
begin
   ...
   for j in att_flt(dFirstDay, dEnd) loop
      ...
      if j.dt_embarked > dFirstDay then
         ...
         -- if less than dFirstDay but greater than 26 of previous month
         if j.dt_embarked >= to_date(to_char(add_months(p_date_fr,-1), 'YYYYMM') || '26', 'YYYYMMDD') then
            -- check previous disembarked 
            begin
               select max(dt_disembarked) 
               into   dLastDisembarked 
               from   cms_voyage_crew 
               where  empl_empl_id = p_empl_id
               and    dt_disembarked is not null 
               and    dt_disembarked < j.dt_embarked;
            exception
               when others then dLastDisembarked := to_date('19010101', 'YYYYMMDD');
            end;
            if dPrevStart is null AND j.dt_embarked < dFirstDay and (dLastDisembarked+1) = j.dt_embarked then   -- EOM-APR... 0101-0325 em/disem then 0326 em
               dStart := dFirstDay;
            else 
               dStart := j.dt_embarked;
            end if;
      ...
      if p_empl_id in (dEmplID) then
         dbms_output.put_line ('check 0a: ' || p_empl_id || ': start=' || to_char(dStart) || ': End=' || to_char(dEnd) || ': dPrevStart=' || to_char(dPrevStart) || ': dFirstDay=' || to_char(dFirstDay) || ': RecCtr=' || to_char(nRecCtr));
      end if;
      -- check end date
      -- check end date
      ...
      then
         ...
         if j.dt_disembarked < p_date_fr then
            ...
            if j.dt_disembarked < dFirstday then         -- EOM-feb... feb 13 disbarked scenario
               dEnd := p_date_fr;
            else
               dEnd := j.dt_disembarked;
            end if;                                      -- EOM-feb... feb 13 disbarked scenario
            if j.dt_embarked > dFirstDay then
               dStart := j.dt_embarked;
            else
               if j.dt_disembarked < p_date_fr then     -- MID-MAR comp... feb 26 disbarked scenario
                  if (dPrevStart-1) = j.dt_disembarked and 
                      j.dt_embarked >= to_date(to_char(add_months(p_date_fr,-1), 'YYYYMM') || '26', 'YYYYMMDD')
                  then                                  -- EOM-APR comp. 03-29 to 04-10 em/disembarked and 4-11 embarked again
                     dStart := j.dt_embarked;           -- EOM-APR comp. 03-29 to 04-10 em/disembarked and 4-11 embarked again
                  else
                     dStart := dFirstDay;               -- MID-MAR comp... feb 26 disbarked scenario
                  end if;
               else
                  dStart := dFirstDay-1;
               end if;
            end if;                                     -- MID-MAR comp... feb 26 disbarked scenario
            -- for feb changes
            vAdjFlag := 'Y';
            bAdjustment := TRUE;
         end if;
      ...
      if nRecCtr >= 1 then
         ...
         if dEnd >= dPrevStart then      -- disembarked gt start dt/latest appt 
            ...
            if dEnd = p_date_fr then     -- disembarked eq to start date
               ...
               if (dPrevStart=to_date(to_char(p_date_fr, 'YYYYMM') || '10', 'YYYYMMDD') and bIsEndOfMonth) or
                  (dEnd=to_date(to_char(p_date_fr, 'YYYYMM') || '10', 'YYYYMMDD') and bIsEndOfMonth and 
                   dPrevStart>to_date(to_char(p_date_fr, 'YYYYMM') || '15', 'YYYYMMDD')
                  ) 
               then                                                   -- service is continous no need to compute, no adjustment
                  vAdjFlag := 'N';
                  bAdjustment := FALSE;
                  bContinuous := TRUE;
                  if dStart > dEnd then
      ...
      if j.empl_empl_id in (dEmplID) then
         dbms_output.put_line ('check 3: ' || p_empl_id || ': start' || to_char(dStart) || ': End=' || to_char(dEnd) || 
                               ...
                               '::check=' || to_char( to_date('26' || to_char(add_months(p_date_fr,-1), 'MMYYYY'), 'DDMMYYYY')));
      end if;
      
      -- scenario: aballe disembarked mar 25 and embarks mar 26. add/deduct sunday's payment
      if (not bIsEndOfMonth) and dStart = to_date('26' || to_char(add_months(p_date_fr,-1), 'MMYYYY'), 'DDMMYYYY') then
         for k in last_flt(dStart-1) loop
            nLastRate := k.basic_rate;
            vLastPosi := k.posi_code;
            vLastTitl := k.title;
            vLastVess := k.voya_vess_code;
            nUpTO := (last_day(dStart)-dStart)+1;
            dDate := dStart-1;
            if j.empl_empl_id in (dEmplID) then
               dbms_output.put_line ('check 3a: dDate=' || to_char(dDate) || ',nUpTO=' || to_char(nUpTO));
            end if;
            for i in 1..nUpTo loop
               dDate := dDate  + 1;
               nLastNumDay := nLastNumDay + 1;
               if sf_is_holiday (dDate) = 1 then
                  if j.empl_empl_id in (dEmplID) then
                     dbms_output.put_line ('check 3a: holiday=' || to_char(dDate));
                  end if;
                  if sf_is_sunday(dDate) = 1 then
                     nLastHolSun := nLastHolSun + 1 ;
                  else
                     nLastRegHol := nLastRegHol + 1;
                  end if;
               else
                  if sf_is_sunday(dDate) = 1 then
                     if j.empl_empl_id in (dEmplID) then
                        dbms_output.put_line ('check 3a: sunday=' || to_char(dDate));
                     end if;
                     nLastRegSun := nLastRegSun + 1;
                  end if;
               end if;
            end loop;
            exit;             
         ...
         if j.empl_empl_id in (dEmplID) then
            dbms_output.put_line ('check 3b: nLastRegSun=' || to_char(nLastRegSun) || ',nLastRegHol=' || to_char(nLastRegHol) || ',nLastHolSun=' || to_char(nLastHolSun) || ',nLastRate=' || to_char(nLastRate));
         end if;
      end if;
      -- scenario: aballe disembarked mar 25 and embarks mar 26. add/deduct sunday's payment
      ...
      begin
         ...
         nLastRegSun:=nLastRegSun*p_Sunday_RF;
         nLastRegHol:=nLastRegHol*p_Holiday_RF;
         nLastHolSun:=nLastHolSun*p_HolSun_RF;
         nNumday := nNumday + (nLastRegSun+nLastRegHol+nLastHolSun);
         
         if j.empl_empl_id in (dEmplID) then
            dbms_output.put_line ('check 3c: nNumday=' || to_char(nNumday));
         end if;
         ...
         if bContinuous then
            ...
            insert into pys_payroll_dtl
                   ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, amt_g, basic_rate_g, vess_code, dept_code, created_by, dt_created, pay_flag, adj_flag, sal_freq, latest_vess, adjusted )
            values ( p_payno, p_year, p_mon, dStart, dEnd, nSeqNo, j.empl_empl_id, j.posi_code, j.title, vPaty, 0, nNumDay, j.basic_rate, 0, decode(j.basic_rate_g,0,j.basic_rate,j.basic_rate_g), j.voya_vess_code, 'FL', user, sysdate, 'ADD', vAdjFlag, p_sal_freq, vLatestVess, 'Y' );
         else
            if dEnd > p_date_fr then
               insert into pys_payroll_dtl
                      ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, amt_g, basic_rate_g, vess_code, dept_code, created_by, dt_created, pay_flag, adj_flag, sal_freq, latest_vess, adjusted )
               values ( p_payno, p_year, p_mon, p_date_fr, dEnd, nSeqNo, j.empl_empl_id, j.posi_code, j.title, vPaty, nSalaryR, nNumDay, j.basic_rate, nSalaryG, decode(j.basic_rate_g,0,j.basic_rate,j.basic_rate_g), j.voya_vess_code, 'FL', user, sysdate, 'ADD', vAdjFlag, p_sal_freq, vLatestVess, 'Y' );
            else
               insert into pys_payroll_dtl
                      ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, amt_g, basic_rate_g, vess_code, dept_code, created_by, dt_created, pay_flag, adj_flag, sal_freq, latest_vess, adjusted )
               values ( p_payno, p_year, p_mon, dStart, dEnd, nSeqNo, j.empl_empl_id, j.posi_code, j.title, vPaty, nSalaryR, nNumDay, j.basic_rate, nSalaryG, decode(j.basic_rate_g,0,j.basic_rate,j.basic_rate_g), j.voya_vess_code, 'FL', user, sysdate, 'ADD', vAdjFlag, p_sal_freq, vLatestVess, 'Y' );
            end if; 
         end if; 
      end;

      if (nLastNumDay+nLastRegSun+nLastRegHol+nLastHolSun) <> 0 then
         nLastNumDay:= nLastNumDay+nLastRegSun+nLastRegHol+nLastHolSun;
         nLastPay   := nLastRate * nLastNumDay;

         nSeqNo := nSeqNo + 1;
         insert into pys_payroll_dtl
               ( pahd_payroll_no, pahd_year, pahd_mo, period_fr, period_to, seq_no, empl_empl_id, posi_code, title, paty_code, amt, no_days, basic_rate, vess_code, dept_code, created_by, dt_created, pay_flag, sal_freq, latest_Vess, adjusted )
         values ( p_payno, p_year, p_mon, dStart, last_day(dStart), nSeqNo, j.empl_empl_id, vLastPosi, vLastTitl, 'REG-ADJ', nLastPay*-1, nLastNumDay*-1, nLastRate, vLastVess, 'FL', user, sysdate, 'ADD', p_sal_freq, vLatestVess, 'Y' );
      end if;
      ...
      if nRecCtr = 0 and nNumDay > 0 then
         -- get allowances (FLT)
         --for x in allo_flt ( j.empl_empl_id, j.voya_vess_code,  j.voya_voyage_date  ) loop
         for x in allo_flt ( j.empl_empl_id, dStart  ) loop
            nSeqNo := nSeqNo + 1;
         ...
         if i.adj_flag ='Y' then
            ...
            if bIsEndOfMonth then
               ...
               if i.no_days > 0 then
                  ...
                  -- if disembarked date falls on assumed days.
               ...
               if i.period_to < p_date_fr then    -- if disembarked date falls on assumed days.
                  select count(1) into nChkReg from pys_payroll_dtl where empl_empl_id = p_empl_id and pahd_payroll_no = p_payno and paty_code = 'REG';
                  if nChkReg = 0 then             -- last day
                     dAStart := i.period_to;
                     dAEnd   := p_date_fr-1;
                     nAUpTO := (dAEnd-dAStart) + 1;
                     dADate := dAStart - 1;
            ...
            update pys_payroll_dtl
            set    period_fr = least(dAStart,dAEnd-1),
                   period_to = dAEnd-1,
            ...
            if (not bIsEndOfMonth) and ((nADays-nANumDay) <> 0)  then
               update pys_payroll_dtl
               set    amt_g   = amt_g + (((nADays-nANumDay)*-1) * basic_rate),
                      amt     = amt   + (((nADays-nANumDay)*-1) * basic_rate),
                      no_days = no_days + ((nADays-nANumDay)*-1)
               where  pahd_payroll_no = i.pahd_payroll_no
               and    paty_code = 'REG'
               and    empl_empl_id = p_empl_id
               and    rownum <= 1;
