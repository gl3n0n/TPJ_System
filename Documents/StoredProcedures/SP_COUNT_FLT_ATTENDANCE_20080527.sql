...
begin  
   ...
   for j in att_flt(dFirstDay, dEnd) loop
      ...
      begin
         ...
         values ( p_payno, p_year, p_mon, dStart, dEnd, nSeqNo, j.empl_empl_id, j.posi_code, j.title, vPaty, nSalaryR, nNumDay, j.basic_rate, nSalaryG, decode(j.basic_rate_g,0,j.basic_rate,j.basic_rate_g), j.voya_vess_code, 'FL', user, sysdate, 'ADD', vAdjFlag );
   ...
   if bAdjustment then
      for i in adj_flt loop
         ...
         if i.adj_flag ='Y' then
            ...
            nAdjPay := ((p_Sunday_RF * nARate) * nARegSun) + 
                       ((p_Holiday_RF * nARate) * nARegHol) +
                       ((p_HolSun_RF * nARate) * nAHolSun) +
                       ((nARate) * nANumDay);
