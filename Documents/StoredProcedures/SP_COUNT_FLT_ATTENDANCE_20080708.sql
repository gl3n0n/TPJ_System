...
begin
   ...
   for j in att_flt(dFirstDay, dEnd) loop
      ...
      if j.dt_embarked > dFirstDay then
         ...
         if j.dt_embarked = to_date(to_char(p_date_fr, 'YYYYMM') || '10', 'YYYYMMDD') then
            dStart := dFirstDay;
         else 
            dStart := j.dt_embarked;
         end if;
      ...
      if nRecCtr >= 1 then
         ...
         if dEnd >= dPrevStart then      -- disembarked gt start dt/latest appt 
            if dPrevStart=dFirstDay then     -- service is continous no need to compute, no adjustment
               vAdjFlag := 'N';
               bAdjustment := FALSE;
               bContinuous := TRUE;
            else 
      ...
      for i in adj_flt loop
         ...
         if i.adj_flag ='Y' then
            ...
            set    period_fr = dAStart,
                   ...
                   amt_g   = nvl(abs(dPrevRateG - decode(sal_freq,'MONTHLY',i.basic_rate_g/30,i.basic_rate_g)) * nADays,0),
                   amt     = nvl(nAdjPay,0),
                   no_days = nADays
