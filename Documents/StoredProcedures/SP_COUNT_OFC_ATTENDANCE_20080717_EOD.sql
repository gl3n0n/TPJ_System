...
begin
   ...
   for j in atre ( p_empl_id, dStart, dEnd ) loop
      ...
      if p_empl_id = dEmplID then
         dbms_output.put_line ('check j.tx_date: ' || to_char(j.tx_date) || ' j.num_hours: ' || to_char(j.num_hours) || ' j.outer_port:' || j.outer_port || ' j.ot_hours:' || to_char(j.ot_hours) || ',nNumHrs:' || to_char(nNumHrs));
      end if;
      nDay  := nDay + 1;
      dDate := j.tx_date;
      nTmpHrs := 0;
   ...
   if nOuterNo > 0 then
      ...
      values ( p_payno, p_year, p_mon, dOuterFr, dOuterTo, nSeqNo, p_empl_id, p_posi_code, 'REG-OP', nOPPay, nOuterNo, nOuter_R, nOPPay, nOuter_R, null, p_dept_code, user, sysdate, 'ADD', vSalFreq );
   end if;
